extends Control

@onready var game_controller = $".."
@onready var lista_magias = $lista_magias

func _ready() -> void:
	atualizar_lista_magias()

func _on_rich_text_label_gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		if self.position.x == 0:
			self.position.x -= 200
			atualizar_lista_magias() # atualiza a lista sempre que o inventário abre
		else:
			self.position.x = 0

func atualizar_lista_magias() -> void:
	# Limpa os itens antigos da lista antes de recriar
	for filho in lista_magias.get_children():
		filho.queue_free()
	
	for i in range(game_controller.inventario_magia.size()):
		var magia = game_controller.inventario_magia[i]
		var item_magia = TextureButton.new()
		item_magia.texture_normal = magia.sprite
		item_magia.tooltip_text = magia.nome
		item_magia.custom_minimum_size = Vector2(64, 64)
		item_magia.position.x = -32 + (i * 70)  # espaça cada item horizontalmente
		item_magia.position.y = -32
		item_magia.scale.x = 0.25
		item_magia.scale.y = 0.25
		lista_magias.add_child(item_magia)
